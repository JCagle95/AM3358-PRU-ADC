...
//misc constants:
#define SCLK_DELAY R15     //Serial clock delay counter

START:

	MOV		PRU_RAM_ADDRESS, PRU_DATA0_BASE
	LBBO 	MAX_SIZE, PRU_RAM_ADDRESS, OFFSET(DataStorage.Delay), 4
	ADD 	SECTOR_ONE_ADDRESS, PRU_RAM_ADDRESS, OFFSET(DataStorage.Delay) + 4
	ADD 	SECTOR_TWO_ADDRESS, SECTOR_ONE_ADDRESS, MAX_SIZE
	LDI		SECTOR_COUNT, 0
   	ADD		SECTOR_TWO_END, SECTOR_TWO_ADDRESS, MAX_SIZE
   	//LBBO 	SCLK_DELAY, PRU_RAM_ADDRESS, OFFSET(DataStorage.Delay), 4
	set CONVST
	clr FS
...
READ_DATA_IN:
    ...
    CONV_DELAY:
		...
		LBBO	DELAY_REG, PRU_RAM_ADDRESS, OFFSET(DataStorage.Delay), 4
